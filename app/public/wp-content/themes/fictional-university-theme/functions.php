...
// Carregar arquivo de configuração com chaves de API
...
function university_files() {
  wp_enqueue_script( 'googleMap', '//maps.googleapis.com/maps/api/js?key=' . GOOGLE_MAPS_API_KEY, array(), '1.0', true );
...
function redirectSubsToFrontend() {
    ...
    if(count($ourUser->roles) == 1 && $ourUser->roles[0] == 'subscriber') {
...
function makeNotePrivate($data, $postarr) {
  if($data['post_type'] == 'note') {
    if(count_user_posts(get_current_user_ID(), 'note') >= 5 && !$postarr['ID']) {
      die('You have reached the limit of 5 notes');
    }
